...
class GAMINet(tf.keras.Model):
    ...
    def local_explain(self, class_, mf_output,x, y=None, save_dict=False, folder="./", name="local_explain"):
        ...
        intercept = self.output_layer.output_bias[class_].numpy()
        ...
        scores = np.hstack([intercept, np.hstack([main_effect_weights, interaction_weights]) 
                                          * np.hstack([main_effect_output, interaction_output]), mf_output])
        try:
            active_indice = 1 + np.hstack([-1, self.active_main_effect_index[class_], self.input_num + self.active_interaction_index[0].astype(int),-2])
        except:
            active_indice = 1 + np.hstack([-1, self.active_main_effect_index[class_], self.input_num + self.active_interaction_index.astype(int),-2])
